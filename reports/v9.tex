...
\jb{main TODO is to kick off the BLEU ablation experiment for section 4, and tighten up the structure of the paper to accommodate}
\jb{Rerun experiments for Aya}
\jb{Describe the takeaways briefly, clarity is the most important. Don't aim for any length---summarizing the trends in a paragraph per table/figure is a rough estimate of what it might come to at the end}
\jb{The methods could use a bit more detail of the methodology, explain to both a translation and mech interp audience, so explain both topics. Define the basic terms + cite the rest. Less detail needed for the prompting scheme.}
\jb{Limitations can be done at the very end}
...
% This means that the middle layers of a model are essentially ``semantic hubs'' \am{cite} containing a few concepts that explain much of the variance in the pretraining corpus.
...
\jb{Because of the updated experiments -> change noisy channel to something else that focuses on task agnosticness}

\jb{Jannik's suggestion: add bolded contributions to this section}
...
\begin{equation}
    \beta_S l_S + \beta_Tl_T + \beta_0 = t_{ST}
\end{equation}
...
\jb{I realized I have a table for this as well, let me copy it over}
...
Model & $R^2$ & MAE \\
\midrule
Multi-BLiMP accuracy --- linear   & 0.250 & 4.86 \\
Multi-BLiMP accuracy --- bilinear & 0.251 & 4.84 \\
\bottomrule
\end{tabular}
\caption{Table of some general results from ~\ref{fig:proxy}}
\label{tab:proxy}
\end{table}

\jb{TODO: add p values / Spearman correlation in ~\ref{tab:proxy}}

\jb{TODO: add the 3 subplots with perplexity, linear model, bilinear model to the appendix. Gives visual interpretation of what the R scores mean.}
...
\jb{Aaron notices that Multi-BLiMP might be saturated, and there's lots of variability in BLEU even at high Multi-BLiMP scores}}
\label{fig:proxy}
\end{figure}
...
\caption{Llama BLEU matrix and its rank-1 SVD reconstruction. The missing cells along the diagonal are imputed by the mean of the target column. \jb{TODO: mask the diagonal out instead of imputation, I think this would overestimate the faithfulness} \jb{This is a little slippery, since the interaction effects may be marginalized, and can still be used in the rank1 approximation. Move to appendix}}
\label{fig:rank1}
\end{figure}
...
\jb{TODO: intuition about patching}

\jb{TODO: ensure that you really want negative indirect effect to mean model components that influence the model to prefer the correct (grammatical) completion}
\begin{equation}
...
\jb{Detail needed for GCM: ~this length is good, but make it actually well-understandable}
...
\jb{TODO: remove this, going with attn heads angle}
\jb{TODO: idea that might not be worth it from Aaron---doing the same procedure but swapping the source instead of target, so that the control tasks are measuring off the same content}
\jb{TODO: add interpretation---the blue bars being almost as high as the red bars indicate that there are many features which simply promote any sample in the target language, without necessarily promoting the \emph{correct} translation}
...
\jb{This could be largely subsumed by the other figures. Maybe it would be better to swap out null\_same for null\_cross and just have the plot be of that, with no color? OR swap for real\_same, but real\_same is kinda measuring copying a little bit instead of monolingual as we'd like it to}
\end{figure}

\jb{TODO: 
Ablations of SAE features -> effect on BLEU
Sort the SAE features by their real\_cross. Progressively ablate more and more SAE features, measuring BLEU scores with more ablations
}

\subsection{Signedness of Indirect Effect of Heads Predicts Monolingual Grammatical Performance}
\jb{TODO: in general, focus on attn heads, SAE stuff is on the back burner}
\jb{TODO: cache some of the generations with and without ablations}
...
\jb{TODO: This table \ref{tab:multiblimp-examples} is from Claude, so will need to verify that this is in fact an actual example, plus clean up the width}
\begin{table}[t]
\centering
\small
...
\begin{equation}
\begin{split}
\Delta &= \log p(\texttt{correct completion} | \texttt{prefix}) \\ & \quad - \log p(\texttt{incorrect completion} | \texttt{prefix})
\end{split}
\end{equation}
...
\caption{
\jb{Two ideas here for an updated figure: we could get the top IE across all language pairs (in everything that we measured) or have some kind of multilinguality constraint as is present currently}
Signed IE of the top 20 universal heads across all 56
...
\jb{The point of this is to show the cross-taskness and multilinguality. Include a condensed version of this (e.g. the top 14 rows)}
\jb{Consider pushing this to the appendix}
...
\jb{Another worry: label how many heads are actually ablated for each pos, neg}
\jb{Run this same experiment, but with BLEU and not Multi-BLiMP accuracy}
...
\jb{This section is sparse because I'm not sure if we'd keep it. But we could give an account of how we derived the grammatical concepts SAE features (input and output) from UD, then show that there is some overlap between those SAE features and the GCM SAE features but Figure ~\ref{fig:gcm-grammar} has some positive evidence for the translation components being responsible for modeling the grammar}
...
\jb{This is probably not very surprising/past results were muddy i.e. the ablations turned out to not have a large effect}
...
\label{sec:monolingual-vs-parallel}

If LLMs translate via language-general internal representations, translation quality should depend in part on the model's monolingual competence in the source and target languages.
Under this view, a translation error need not reflect a missing pair-specific mapping: it may instead reflect a failure to encode the source sentence into an adequate internal representation, or to decode the target sentence fluently from it.
This suggests that improvements in monolingual language modeling should, at least in part, transfer to translation---even when using no parallel data.

We test this prediction by comparing monolingual and parallel LoRA fine-tuning on the Llama-3.1-8B base model.
We focus on Xhosa, where the base model has substantially weaker language modeling and translation performance than on other high-resource languages.
The parallel runs train on 200K Xhosa--English sentence pairs; the monolingual runs train only on 200K samples of Xhosa text.
For both settings, we run a small hyperparameter sweep over learning rate, LoRA rank, and number of training examples, and select the checkpoint with the highest Xhosa$\rightarrow$English BLEU.

\paragraph{Translation quality.}
Table~\ref{tab:mono-vs-parallel-bleu} reports BLEU for the base model, the strongest parallel run in our sweep, and the strongest monolingual run.
The best parallel run yields a modest gain over the base model, from 12.10 to 12.96 Xhosa$\rightarrow$English BLEU.
The best monolingual run reaches 17.72 BLEU, substantially exceeding the strongest parallel adaptation we found despite using no translation supervision.
...
\begin{tabular}{lrr}
\hline
Model & Xho$\rightarrow$Eng & Eng$\rightarrow$Xho \\
\hline
Base & 12.10 & 1.04 \\
Best parallel FT & 12.96 & -- \\
Best mono Xho FT & 17.72 & 1.48 \\
\hline
\end{tabular}
\caption{BLEU for non-instruct Llama-3.1-8B LoRA runs. The best monolingual Xhosa run substantially outperforms the best parallel run found in our sweep on Xhosa$\rightarrow$English.}

\jb{TODO: check on the parallel FT for Eng $\to$ Xho}

\jb{Aaron has the idea of adding more language pairs here, and seeing if we could do better about eliciting the parallel data}
\label{tab:mono-vs-parallel-bleu}
\end{table}

While we do not claim that monolingual fine-tuning dominates parallel fine-tuning in general, we find that direct translation supervision was not necessary to obtain the improvements in translation performance.
This is consistent with the hypothesis that gains in source-language competence transfer to translation via shared internal representations.

\paragraph{Perplexity.}
Table~\ref{tab:mono-vs-parallel-ppl} clarifies the underlying tradeoff.
The monolingual run reduces Xhosa perplexity from 66.70 to 26.23, while the best parallel run leaves it essentially unchanged at 71.04.
This supports the interpretation that the monolingual BLEU gain reflects improved Xhosa language modeling competence.
The monolingual run, however, substantially degrades English, French, and German perplexity, whereas the parallel run preserves them.
...
\begin{tabular}{lrrrr}
\hline
Model & Xho PPL & Eng PPL & Fra PPL & Deu PPL \\
\hline
Base & 66.70 & 21.86 & 11.25 & 12.50 \\
Best parallel FT & 71.04 & 23.42 & 11.20 & 13.36 \\
Best mono Xho FT & 26.23 & 40.63 & 34.72 & 34.05 \\
\hline
\end{tabular}
\caption{Perplexity for the same runs. Monolingual fine-tuning produces a much larger improvement on Xhosa, but degrades the non-Xhosa languages that parallel fine-tuning preserves.}
\label{tab:mono-vs-parallel-ppl}
\end{table}

Together, these results point to a specialization--preservation tradeoff: monolingual adaptation alone is sufficient to produce a large Xhosa$\rightarrow$English gain, but it does so by specializing the model toward Xhosa at the expense of other languages.
A practical recipe would likely combine monolingual adaptation with non-target-language replay to retain the translation gains without broader multilingual degradation.
...
\section*{Limitations}
We're only looking at 2 models of similar size
...
\jb{TODO: Inline you can say something about having a 2-shot prompt, but not much more elaboration is needed. Written by Claude:}
